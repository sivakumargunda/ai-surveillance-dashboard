...
def _save_alert_snapshot(
    ...
    annotated = annotator.draw(frame.copy(), detections, events, 0, _zones_for(analyzer.zone_rect))
...
def main() -> None:
    ...
    print("  alerts  : database enabled")
    print("  stream  : MJPEG enabled at /stream/{camera_id}")
    print("=" * 60)
    ...
    with source:
        for frame in source:
            ...
            if frame.frame_number % 2 == 0:
                annotated = annotator.draw(frame.data.copy(), tracked, emitted, fps, _zones_for(analyzer.zone_rect))
            ...
            if ENABLE_ZONE_CLIPS:
                ...
                for event in events:
                    ...
                    if writer.isOpened():
                        active_clip_writers[event_id] = {"writer": writer, "path": clip_path}
            ...
            if snapshot_events:
                saved = _save_alert_snapshot(frame.data, tracked, snapshot_events, analyzer, annotator)
